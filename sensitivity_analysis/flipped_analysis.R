...
results <- expand.grid(
  ...
  dplyr::mutate(
    ...
    global_weights_00 = purrr::map2(
      xg_limit, distance_limit,
      ~ readRDS(glue::glue("flipped_00_global_weights_{.x}_{.y}.rds"))
    ),
    ...
    global_weights = purrr::map2(
      xg_limit, distance_limit,
      ~ readRDS(glue::glue("flipped_01_global_weights_{.x}_{.y}.rds"))
    ),
    player_weights = purrr::map2(
      xg_limit, distance_limit,
      ~ readRDS(glue::glue("flipped_01_player_weights_{.x}_{.y}.rds"))
    ),
