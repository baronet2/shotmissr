...
for (xg_limit in c(0.1, 0.5, 1)) {
  for (distance_limit in c(0, 6, 15)) {
    ...
    saveRDS(global_weights, glue::glue("flipped_00_global_weights_{xg_limit}_{distance_limit}.rds"))
    ...
    saveRDS(
      ...
      glue::glue("flipped_01_global_weights_{xg_limit}_{distance_limit}.rds")
    )

    saveRDS(
      mixture_model_fit[["player_weights"]],
      glue::glue("flipped_01_player_weights_{xg_limit}_{distance_limit}.rds")
    )
  }
}
