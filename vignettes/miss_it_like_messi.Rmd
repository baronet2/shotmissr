...
pipeline <- function(
  ...
  shooting_skill_data <- shots_data |>
    dplyr::group_by(dplyr::all_of(grouping_cols)) |>
  ...
  player_rb_post_xg <- data.frame(
    group_id = 1:max(shooting_skill_data$group_id),
    player_rb_post_xg = player_weights %*% mixture_model_components$value
  )
  
  shooting_metrics <- shooting_skill_data |>
    dplyr::inner_join(player_rb_post_xg, by = "group_id") |>
